...
`session_id` doubles as a **project namespace**. Always set it to the project slug (e.g. `"taty-v2"`, `"korm-v2"`) when ingesting, storing, or retrieving project-specific content. This scopes Qdrant retrieval to that project only and prevents cross-project contamination.

- **Ingesting a codebase** → `session_id = "<project-name>"` (e.g. `"taty-v2"`)
- **Storing a project fact** → `session_id = "<project-name>"`
- **Retrieving for a project** → `session_id = "<project-name>"` to get only that project's chunks
- **Cross-project or global facts** → omit `session_id` or use a descriptive slug (e.g. `"user-preferences"`)
...
1. User opens a new chat → call `context_retrieve` with the first message as the query and `session_id` set to the active project slug
2. Inject retrieved context into your reasoning before responding
3. During the conversation, call `context_store` with the project slug as `session_id` whenever a decision or fact is established
